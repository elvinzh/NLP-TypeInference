...
keywordpath1 = "./data/keywords1.txt" 
keywordpath2 = "./data/keywords2.txt"
...
def top_k_res(pred_,k=3):
    res = []
    for i in range(pred_.shape[0]):
        res.append(top_k_acc(pred_[idd],Y_test[i], k=k))
    acc = sum(res)/pred_.shape[0]
    return acc


def top_k_acc(pred,label, k=3):
    res = 0
    reh = predi.reshape(150)
    xx = np.argsort(reh)
    idx = 0
    while idx < k:
        idx += 1
...
labels = ['Lstm','OCamel', 'SHErrLoc', 'Pretained Bert', 'Init Bert', 'CodeBert', 'Nate']
sp14 = [0.1246, 0.45, 0.55, 0.5892, 0.4657, 0.5597, 0.61]
fa15 = [0.1103, 0.43, 0.56, 0.5816, 0.4415, 0.5546, 0.595]
...
rects2 = ax.bar(x + width/2, fa15, width, label='fa15')
